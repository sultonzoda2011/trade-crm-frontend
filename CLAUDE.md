...
- **Auth is cookie-based — nothing attaches a Bearer header.** The backend sets an httpOnly `accessToken` cookie plus a readable `user` cookie; the browser sends them via `withCredentials`. On 401 the response interceptor surfaces the error and SPA-navigates to `/login`. Identity on the client comes from `getClientUser()` reading the `user` cookie — not from `jwt-decode`.
